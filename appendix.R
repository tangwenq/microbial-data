...
# The following code will do the simulation when true models are NB-GLLVM and NB-GCLVM

# True model: NB-GCLVM

# fit the NB_GCLVM model with subset of data to obtain true model and true ordinations.
fit_NBcopula <- function(data, m_species) {
  # Select the first m microbiome data
  y <- data[, 1:m_species]
  
  true_mod <- fit_copula(y, reff = "fixed", gllvm.fam = "negative.binomial", seed = 123, lv.n = 0)
...
# a) species/col = 50
copula_50 <- fit_NBcopula(data, m_species = 50)

sim50_copula <- sim_copula(copula_50, rep_k = 50)

write.table(sim50_copula, file = "sim50_copulaNB.txt")

# b) species/col = 100
copula_100 <- fit_NBcopula(data, m_species = 100)

sim100_copula <- sim_copula(copula_100, rep_k = 50)


write.table(sim100_copula, file = "sim100_copulaNB.txt")
...
write.table(sim200_copula, file = "sim200_copulaNB.txt")
...
write.table(sim400_copula, file = "sim400_copulaNB.txt")

# Plot Procrustes errors boxplot under each dimension, shown in Figure 8 in appendix
...
pdf(file = "copula_NB.pdf", width = 5, height = 5, useDingbats = FALSE)
...
# fit the NB_GLLVM model with subset of data to obtain true model and true ordinations.
fit_gllvm <- function(data, m_species, seed_NO) {
  # Select the first m microbiome data
  y <- data[, 1:m_species]
  
  
  true_mod <- gllvm(y, family = "negative.binomial", sd.errors = FALSE, row.eff = "fixed", seed = seed_NO)
...
write.table(sim50_gllvm, file = "sim50_gllvmNB.txt")
...
write.table(sim100_gllvm, file = "sim100_gllvmNB.txt")
...
write.table(sim200_gllvm, file = "sim200_gllvmNB.txt")
...
write.table(sim400_gllvm, file = "sim400_gllvmNB.txt")


# Plot Procrustes errors boxplot under each dimension, shown in Figure 7 in appendix
